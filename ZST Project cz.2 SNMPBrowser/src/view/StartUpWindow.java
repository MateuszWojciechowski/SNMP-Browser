...
public class StartUpWindow extends JFrame {
	...
	private JTextField writeCommunity;
	...
	public StartUpWindow() {
		...
		writeCommunity = new JTextField();
		writeCommunity.setForeground(Color.WHITE);
		writeCommunity.setFont(new Font("Tahoma", Font.BOLD, 11));
		writeCommunity.setColumns(10);
		writeCommunity.setBackground(Color.LIGHT_GRAY);
		writeCommunity.setBounds(101, 63, 106, 20);
		contentPane.add(writeCommunity);
		...
		JTextField txtWriteCommunity = new JTextField();
		txtWriteCommunity.setText("Write Community");
		txtWriteCommunity.setHorizontalAlignment(SwingConstants.RIGHT);
		txtWriteCommunity.setForeground(Color.WHITE);
		txtWriteCommunity.setFont(new Font("Tahoma", Font.BOLD, 10));
		txtWriteCommunity.setEnabled(false);
		txtWriteCommunity.setEditable(false);
		txtWriteCommunity.setColumns(10);
		txtWriteCommunity.setBackground(Color.GRAY);
		txtWriteCommunity.setBounds(0, 63, 102, 20);
		...
		txtSnmpVersion.setBounds(0, 84, 102, 20);
	...
	protected void getServerIP() throws IOException {
		...
		try (FileReader reader = new FileReader("snmp.properties")) {
			...
			writeCommunity.setText(prop.getProperty("writeCommunity"));
	...
	private void okActionPerformed() {
		...
		SNMPConnectionConfiguration conf = new SNMPConnectionConfiguration(machineAddress.getText(),
				Integer.valueOf(snmpPort.getText()), readCommunity.getText(), writeCommunity.getText(),
				Integer.valueOf(snmpVersion.getText()));
